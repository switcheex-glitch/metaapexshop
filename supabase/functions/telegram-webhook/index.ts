    ...
    if (action === 'ok' || action === 'ji_ok') {
      ...
      if (isJI) {
        ...
        if (tierData) {
          ...
          console.log(`[telegram-webhook] Tier ${purchase.tier}: ${tokensCount} tokens, ${durationDays} days`);
        } else {
          console.warn(`[telegram-webhook] Tier not found: ${purchase.tier}, using purchase.tokens=${tokensCount}`);
        }
      }
